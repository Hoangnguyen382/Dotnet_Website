    ...
    namespace DoAn_WebAPI.Services
    {
        public class UserService : IUserService
        {
            ...
            public async Task<User?> CreateUserAsync(UserDTO user)
            {
                ...
                var verifyLink = $"{baseUrl}/verify-email?token={Uri.EscapeDataString(token)}";
            ...
            public async Task<User?> CreateStaffAccountAsync(UserDTO dto, int createdByUserId)
            {
                var creator = await _userRepository.GetUserByIdAsync(createdByUserId);
                if (creator == null)
                    throw new UnauthorizedAccessException("Người tạo không tồn tại.");

                if (creator.Role != "Admin" && creator.Role != "Owner")
                    throw new UnauthorizedAccessException("Bạn không có quyền tạo tài khoản nhân viên.");
